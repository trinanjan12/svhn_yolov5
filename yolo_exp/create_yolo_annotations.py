...
parser.add_argument("--mode", default = "train",  type=str , help="mode of folder train/combined")
args = parser.parse_args()

mode = args.mode
anno_file = '../org_dataset/' + mode + '_annotations_processed.pkl'
with open(anno_file, 'rb') as f:
    annotations = pickle.load(f)

output_dir = './svhn2020'+mode
...
print(len(val_img_name_keys))


print('---------------- creating yolo annotations for training ----------------')
print('number of images to be processed for training-->', len(train_img_name_keys))

for image_name in tqdm(train_img_name_keys):
    img_index = int(image_name.split('.')[0])
    img_path = '../org_dataset/' + mode + '/' + image_name
...
print('---------------- creating yolo annotations for validation ----------------')
print('number of images to be processed for validation-->', len(val_img_name_keys))
